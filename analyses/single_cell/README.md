...
The preprocessing workflow in `scripts/preprocess_raw_counts_reference.py` applies per-sample Scrublet filtering, 
the study quality-control thresholds, normalization, highly variable gene selection and principal component analysis. 
After quality control and feature selection, the processed dataset contained 
191,563 cells, 37,487 genes, 2,996 highly variable genes after exclusion of mitochondrial 
and ribosomal genes, and 50 principal components.

The six annotated AnnData objects listed below are required for the downstream statistical analyses.
...
For count-matrix preprocessing, `matrix_dir` specifies 
the sample's 10x-format matrix directory relative to `SLEMODEL_SCRNA_MATRIX_ROOT`; 
when omitted, the script uses `<sample_id>.matrix`.

The AnnData objects must contain a sample identifier in `obs`, 
raw counts in a layer suitable for pseudobulk analysis and normalized log-expression for module scoring. 
The selected layers are listed in the analysis output tables.
...
Scrublet was applied separately to each sample. Cells with 200–6,000 detected genes, 
fewer than 40,000 UMIs, less than 20% mitochondrial transcripts and a Scrublet score below 0.25 were retained. 
Counts were normalized to 10,000 per cell and log1p-transformed. 
Three thousand highly variable genes were selected using the Seurat v3 method, followed by exclusion of mitochondrial and ribosomal genes and calculation of 50 principal components.

The preprocessing script uses a Scrublet expected doublet rate of 0.05 by default;
this value can be changed through a command-line argument:
...
The following scripts generate the single-cell figures from the analysis output tables:
...
These scripts use statistical output tables generated by the single-cell analysis. 
Their input locations can be set with
