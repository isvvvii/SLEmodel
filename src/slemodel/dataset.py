...
class StaticMatchedDataset(Dataset):
    """Validation dataset with fixed barycentric pseudo-pairs."""

    def __init__(self, gly_data, gly_labels, all_mass_data, all_rna_data, mass_coupling, rna_coupling, gly_ids):
...
class DynamicHybridDataset(Dataset):
    ...
    def resample_and_match(self):
        ...
        mass_coupling = get_ot_coupling(
            self.ps_gly, ps_mass_subset,
            reg=self.reg_mass, method=self.ot_method_mass,
        ...
        rna_coupling = get_ot_coupling(
            self.ps_gly, ps_rna_subset,
            reg=self.reg_rna, method=self.ot_method_rna,
